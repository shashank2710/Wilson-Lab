% puprisa_viewChannelMosaic
% Jesse Wilson (2011) syrex314@gmail.com
% Duke University
%
% View a single channel as a mosaic
%
% A visualization tool for mosaic for a single channel. Stitches together
% the mosaic, and displays the entire image at once
%
% requires x, y calibration
%

function puprisa_viewChannelMosaic(imageStack, xPos, yPos, zPos, fileName, header, chan)
    ...
    yPos_px = yPos * scaleY_px_per_micron;
        ...
        %puprisa_scaleBar(100,scaleX_px_per_micron);
    imageStack = flipdim(imageStack,1);
    X = puprisa_stitchMosaic(imageStack,xPos_px,yPos_px, zPos);
    
    puprisa_viewChannel( X, zPosMosaic, fileName, header, chan );
    return;
    ...
    setappdata(gcf,'xPos',xPos);
    setappdata(gcf,'yPos',yPos);
